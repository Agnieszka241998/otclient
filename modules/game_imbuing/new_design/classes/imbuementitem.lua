...
function itemApi.configureWindow(window)
    ...
    for i = 1, 3 do
        ...
        if slotWidget then
            slotWidget.resource:setImageSource("/game_imbuing/images/icons/0")
            if i <= self.slots then
                ...
                if imbuement and imbuement[1] then
                    if imbuement[1].id and imbuement[1].id ~= 0 then
                        slotWidget.resource:setImageSource("/game_imbuing/images/icons/" .. imbuement[1]["imageId"])
                    end
                end
            else
...
function itemApi.onSelectSlotClear(imbuement)
    ...
    widget.resource:setImageSource("/game_imbuing/images/icons/" .. imbuement[1]["imageId"])
...
function itemApi.selectBaseType(selectedButtonId)
    ...
    for id, imbuement in pairs(self.availableImbuements) do
        ...
        if imbuementType == selectedBaseType then
            ...
            widget.resource:setImageSource("/game_imbuing/images/icons/" .. imbuement.imageId)
